---
title: "Compile CNV calls from the 3 capture batches - manual correction"
...
cnv_BATCH1_file = here::here('output','mDiGS_CNV', paste('manual_cnv_correction' , 'BATCH_1' , 'ALL_GENES_COMPILED_filtered_SMOOTHED_CNV_copy_number_calls_TWO_STAGE_CNV_FILTER.txt', sep='_') )
...
cnv_BATCH3_file = here::here('output','mDiGS_CNV', paste('manual_cnv_correction' , 'BATCH_3' , 'ALL_GENES_COMPILED_filtered_SMOOTHED_CNV_copy_number_calls_TWO_STAGE_CNV_FILTER.txt', sep='_') )
...
joint_cnv = select(
  joint_cnv, !c(chrom.y,arm.y,start.pos.y,end.pos.y,n.probes.y,GENE.y)) %>%
...
WHO_HAS_summarised = summarise(WHO_HAS_group_cnv,'NON_TWO_COPY_SAMPLES' = list(unique(SAMPLE[CNV_COPIES != 2])))
...
DISTRIBUTIONS_filename = 'manual_correction_ALL_BATCHES_COMPILED_cnv_distributions.txt'

write.table( CNV_DISTRIBUTIONS_final_output,
             file=here::here('results','mDiGS_CNV', DISTRIBUTIONS_filename ),
             quote=FALSE, 
             sep='\t', 
             row.names=FALSE)

PCT_DIST_filename = 'manual_correction_ALL_BATCHES_COMPILED_cnv_PCT_distributions.txt'
...
WHO_HAS_filename =  'manual_correction_ALL_BATCHES_COMPILED_who_has_CNV.txt'
